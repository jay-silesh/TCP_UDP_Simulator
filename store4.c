...
/* PORT NOs FOR THE STORE4 TO USE AND SEND */
char port_read[2][100]={"17350","19350"};
char port_write[2][100]={"7350","32350"};
char port_self[2][100]={"18350","20350"};
...
int main(int argc, char *argv[])
{
	...
	printf("\nPhase 1: store_4 has TCP port number %d and IP address %s",sock_to_port(sockfd),s);
	...
	/* the files that should be opened for the store4 */
	FILE *fd;
	fd = fopen("Store-4.txt", "r");
	if (fd == NULL)
	{
	...
	ele.storeno=4;
	printf("\nPhase 1: The outlet vector<%d,%d,%d> for store_4 has been sent to the central warehouse",ele.camera,ele.laptop,ele.printer);
...
while(comp_counter!=2)
{
   struct sockaddr_storage their_addr;
    ...
    printf("\nPhase 2: Store_4 has UDP port %d and IP address %s",sock_to_port(sockfd),s1);
	...
	printf("\nPhase 2: Store_4 received the truck-vector<%d,%d,%d> from the Store_3",temp_ele->camera,temp_ele->laptop,temp_ele->printer);
...
/*					sending the truck vector to the second client...										 */
	...
	printf("\nPhase 2: Store_4 has UDP port %d and IP address %s",sock_to_port(sockfd),s2);
	if(comp_counter==0)
	printf("\nPhase 2: The updated truck-vector<%d,%d,%d> has been sent to store_1",temp_ele->camera,temp_ele->laptop,temp_ele->printer);
	else
	printf("\nPhase 2: The updated truck-vector<%d,%d,%d> has been sent to central warehouse",temp_ele->camera,temp_ele->laptop,temp_ele->printer);
	printf("\nPhase 2: Store_4 updated outlet-vector is <%d,%d,%d>",ele.camera,ele.laptop,ele.printer);
